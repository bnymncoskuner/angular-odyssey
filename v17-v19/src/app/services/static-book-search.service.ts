...
export class StaticBookSearchService extends BookSearchService {
  ...
  private loadStaticData(): void {
    if (!this.dataLoaded) {
      this.httpClient.get<GoogleBooksApiResponse>('./api/books.json').subscribe(data => {
  ...
  override search(query: string): void {
    ...
    this.books.set(page);
    this.totalItems.set(this.filteredBooks.length);
    this.error.set(null);
  ...
  override loadMore(): void {
    ...
    this.books.update(existing => [...existing, ...nextPage]);
  }
